  require(RXshrink)
  # read in Portland cement data of Hald(1952).
  data(haldport)
  ...
  trumse <- true.risk(form, data=haldport, trugam, trusig, Q=-5)
  trumse
  plot(trumse)
  cat("\n Press ENTER to simulate True Squared Error Loss...")
  scan()
  trusim <- true.simu(form, data=haldport, trugam, trusig, Q=-5)
  trusim
  plot(trusim)
  ...
  rxsobj <- qm.ridge(form, data=haldpsim)
  rxsobj
  plot(rxsobj)